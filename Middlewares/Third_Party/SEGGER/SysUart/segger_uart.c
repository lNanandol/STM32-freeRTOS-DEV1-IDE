...
#define UART_BASECLK        OS_FSYS / 2       // USART6 runs on APB2 clock
#define GPIO_BASE_ADDR      GPIOC_BASE_ADDR
#define USART_BASE_ADDR     USART6_BASE_ADDR
#define GPIO_UART_TX_BIT    6                // USART6 TX: Pin pc6
#define GPIO_UART_RX_BIT    7                 // USART6 RX: Pin pc7
...
void HIF_UART_Init(uint32_t Baudrate, UART_ON_TX_FUNC_P cbOnTx, UART_ON_RX_FUNC_P cbOnRx) {
  ...
  uint32_t Div;
  //
  // Configure USART RX/TX pins for alternate function AF8
  //
  RCC_APB2ENR |= (1 <<  5);        // Enable USART6 clock
  RCC_AHB1ENR |= (1 <<  2);        // Enable IO port c clock
  v  = GPIO_AFRL;
  v &= ~((15UL << ((GPIO_UART_TX_BIT) << 2)) | (15UL << ((GPIO_UART_RX_BIT) << 2)));
  v |=   ((8UL << ((GPIO_UART_TX_BIT) << 2)) | (8UL << ((GPIO_UART_RX_BIT) << 2)));
  GPIO_AFRL = v;
  //
  ...
  v |=  ((2UL << (GPIO_UART_TX_BIT << 1)) | (2UL << (GPIO_UART_RX_BIT << 1)));         // PA6 e PA7: alternate function
  GPIO_MODER = v;
  //
  // Initialize USART
  //
